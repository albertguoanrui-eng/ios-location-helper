...
(function(){
var source = /\/wloc-helper-probe\/(?:\?|$)/.test($request.url || '') ? 'probe' : /\/clls\/wloc(?:\?|$)/.test($request.url || '') ? 'wloc' : 'observe' === 'panel' ? 'control' : 'other';
function trace(status, error) {
  ...
  try { if ($persistentStore.write(JSON.stringify(event),'location_helper_v1:runtime:observe:'+source) === false) throw Error(); }
  catch (_) { try { console.log('[LocationHelper 0.1.3] observe '+source+' storage-unavailable'); } catch (_) {} }
  if (status === 'failed') { try { console.log('[LocationHelper 0.1.3] observe '+source+' failed '+event.error); } catch (_) {} }
}
var result = {};
trace('entered');
try {
...
return module.exports;})();
  ...
  result = helper.isProbe($request) ? helper.probe($request,(function(){var module={exports:{}};
/*
...
return module.exports;})()) : helper.observe($request);
  trace('completed');
} catch (e) {
  trace('failed', e);
