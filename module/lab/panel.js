// Shadowrocket Location Helper 0.1.3 — AGPL-3.0
// Source: https://raw.githubusercontent.com/albertguoanrui-eng/ios-location-helper/main/module/lab/source.zip
(function(){
var source = /\/wloc-helper-probe\/(?:\?|$)/.test($request.url || '') ? 'probe' : /\/clls\/wloc(?:\?|$)/.test($request.url || '') ? 'wloc' : 'panel' === 'panel' ? 'control' : 'other';
function trace(status, error) {
  var event = {version:'0.1.3',at:Date.now(),status:status};
  if (error) event.error = ['Error','TypeError','ReferenceError','SyntaxError','RangeError'].indexOf(error.name) >= 0 ? error.name : 'Error';
  try { if ($persistentStore.write(JSON.stringify(event),'location_helper_v1:runtime:panel:'+source) === false) throw Error(); }
  catch (_) { try { console.log('[LocationHelper 0.1.3] panel '+source+' storage-unavailable'); } catch (_) {} }
  if (status === 'failed') { try { console.log('[LocationHelper 0.1.3] panel '+source+' failed '+event.error); } catch (_) {} }
}
var result = {};
trace('entered');
try {
  var lib = (function(){var module={exports:{}};
'use strict';
