...
(function(){
var source = /\/wloc-helper-probe\/(?:\?|$)/.test($request.url || '') ? 'probe' : /\/clls\/wloc(?:\?|$)/.test($request.url || '') ? 'wloc' : 'rewrite' === 'panel' ? 'control' : 'other';
function trace(status, error) {
  ...
  try { if ($persistentStore.write(JSON.stringify(event),'location_helper_v1:runtime:rewrite:'+source) === false) throw Error(); }
  catch (_) { try { console.log('[LocationHelper 0.1.3] rewrite '+source+' storage-unavailable'); } catch (_) {} }
  if (status === 'failed') { try { console.log('[LocationHelper 0.1.3] rewrite '+source+' failed '+event.error); } catch (_) {} }
}
var result = {};
trace('entered');
try {
...
return module.exports;})();
  ...
  result = helper.rewrite($request,$response,(function(){var module={exports:{}};
/*
