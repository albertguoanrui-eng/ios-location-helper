...
> **已归档 · 2026-09-11 · 停止维护。** 本项目未实现测试设备上 iOS 27.0 RC 的系统定位修改。面板与人工自检成功不代表定位成功；系统诊断高度指向 Apple 专用证书校验阻碍。代码和历史模块仅供研究，不再建议按下方教程反复尝试。详见 [归档原因与资料说明](docs/ARCHIVED.zh-CN.md)。
...
## 0.1.3 独立实验版：继续排查真实定位未变

**尚未在 iOS 27 RC 真机验证定位成功。** 原 `module/` 的 0.1.2 文件保持不变；实验产物单独发布到 `module/lab/`，不要同时启用两版。

本次新增：匹配默认显式端口 `:443` / `:80`；指定 JavaScriptCore 引擎进行兼容性实验；记录脚本进入、完成和异常；在实际日志出现的中国区定位域名上运行人工自检。人工自检使用合成二进制数据和独立配置，不会伪造“收到系统定位请求”或“定位成功”。

[按步骤使用 0.1.3 实验版](docs/experiment-0.1.3.zh-CN.md)。下面的 0.1.2 教程仍对应原版。
...
用户反馈：0.1.1 面板持续显示“模块接口不兼容”，但直接打开状态接口返回合法 JSON。此前的 HTTP 状态修正没有解决这一持续故障，目前尚未确认自动请求所得内容与直接打开接口的差异。

0.1.2 将当前配置和诊断随面板 HTML 一起返回，打开页面不再自动 fetch 状态接口。“刷新诊断”重新加载整页；保存、停止各提交一次后重新加载，读取持久化状态。沿用原存储键，保留已有坐标。增加具体字段校验和不含坐标的连接诊断，连接失败时也能复制。

1. 在小火箭停用旧定位助手，导入并启用下面的 **0.1.2 CDN 模块**，只保留一个定位模块启用。
2. 重新连接小火箭，在 Safari 打开 `https://gs-loc.apple.com/wloc-helper/?v=0.1.2`。
3. 确认右上角为 **0.1.2**。若仍失败，复制“连接诊断”，不要反复重装或保存坐标。

```text
https://cdn.jsdelivr.net/gh/albertguoanrui-eng/ios-location-helper@main/module/location-helper-cdn-0.1.2.sgmodule
```

这是针对已知可用页面读取路径的调整，通过本地测试，尚未通过该手机实测；不代表已经解决 iOS 系统定位限制。若显示“设置已保存，等待定位请求”，仅代表控制设置可读，仍需观测 WLOC 和实际地图蓝点。
